...
type SortOption = { value: SortValue; label: string; col?: "created_at" | "athlete_name"; asc?: boolean };
...
/** Urutan logis KU — BUKAN alphabetical: tahun lahir (mis. "2019") < level Romawi ("I".."V"); format tak dikenal selalu di akhir. */
function kuSortKey(ku: string): [number, number] {
  ...
  return [2, 0]; // tak dikenal → konsisten di bagian akhir
}
...
export default async function RegistrationsPage({
  ...
  const sortParam = (searchParams.sort ?? "newest") as SortValue;
  const sortOpt = SORT_OPTIONS.find((s) => s.value === sortParam) ?? SORT_OPTIONS[0];
  ...
  if (useMemSort && count != null && count > PAGE_SIZE) {
    let allQ = supabase
      .from("event_payments")
      .select("id, transaction_id, registration_id, athlete_id, athlete_name, cakra, registration_fee, admin_fee, total_amount, amount_paid, remaining_amount, payment_status, payment_method, created_at, event:events(id,name), registration:event_registrations(ku,status)")
      .order("created_at", { ascending: false });
    if (q) allQ = allQ.ilike("athlete_name", `%${q}%`);
    if (fEvent !== "ALL") allQ = allQ.eq("event_id", fEvent);
    if (fCakra !== "ALL") allQ = allQ.eq("cakra", fCakra);
    if (fKu !== "ALL") allQ = allQ.eq("registration.ku", fKu);
