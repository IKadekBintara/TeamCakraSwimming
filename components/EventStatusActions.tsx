...
export default function EventStatusActions({ eventId, status }: { eventId: string; status: string }) {
  ...
  return <div className="flex flex-wrap gap-2 [&>button]:flex-1 [&>button]:sm:flex-none">{status === "DRAFT" && <button disabled={saving} onClick={() => update("OPEN")} className="btn-primary text-sm">Buka Pendaftaran</button>}{status === "OPEN" && <button disabled={saving} onClick={() => update("CLOSED")} className="btn-secondary text-sm">Tutup Pendaftaran</button>}{status !== "CANCELLED" && <button disabled={saving} onClick={() => update("CANCELLED")} className="rounded-lg bg-red-50 px-3 py-2 text-sm font-medium text-red-700 dark:bg-red-950/40 dark:text-red-300">Batalkan Event</button>}{error && <span className="text-xs text-red-700">{error}</span>}</div>;
}
